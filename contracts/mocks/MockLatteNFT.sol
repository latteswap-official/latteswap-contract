...
contract MockLatteNFT is ERC721Pausable, Ownable, AccessControl {
  ...
  mapping(uint256 => string) public latteNames;

  mapping(uint256 => Category) public categoryInfo;

  mapping(uint256 => uint256) public latteNFTToCategory;
  ...
  constructor(string memory _baseURI) public ERC721("LATTE", "LATTE NFT") {
  ...
  function currentCategoryId() public view returns (uint256) {
  ...
  /// @param _categoryId category id
  function categoryURI(uint256 _categoryId) external view onlyExistingCategoryId(_categoryId) returns (string memory) {
   ...
   */
  function getLatteNameOfTokenId(uint256 _tokenId) external view returns (string memory) {
  ...
  ) public virtual onlyMinter onlyExistingCategoryId(_categoryId) returns (uint256) {
  ...
  function mintBatch(
    ...
    string calldata _tokenURI,
    uint256 _size
  ) external onlyMinter onlyExistingCategoryId(_categoryId) returns (uint256[] memory tokenIds) {
