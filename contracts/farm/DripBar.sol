...
contract DripBar is OwnableUpgradeable, ReentrancyGuardUpgradeable  {
    ...
    function addRewardInfo(uint256 _campaignID, uint256 _endBlock, uint256 _rewardPerBlock) external onlyOwner {
        ...
        require(rewardInfo.length == 0 || rewardInfo[rewardInfo.length - 1].endBlock >= block.number, "DripBar::addRewardInfo::reward period ended");
        require(rewardInfo.length == 0 || rewardInfo[rewardInfo.length - 1].endBlock < _endBlock, "DripBar::addRewardInfo::bad new endblock");
    ...
    function emergencyRewardWithdraw(uint256 _campaignID, uint256 _amount, address _beneficiary) external onlyOwner nonReentrant {
        ...
        require(currentStakingPendingReward.add(_amount) <= campaign.totalRewards, "DripBar::emergencyRewardWithdraw::not enough reward token");
