...
contract LatteSwapPair is LatteSwapLP {
  ...
  function _mintFee(uint112 _reserve0, uint112 _reserve1) private returns (bool feeOn) {
    ...
    if (feeOn) {
      if (_kLast != 0) {
        uint256 rootK = LatteSwapMath.sqrt(uint256(_reserve0).mul(_reserve1));
        uint256 rootKLast = LatteSwapMath.sqrt(_kLast);
        if (rootK > rootKLast) {
  ...
  function mint(address to) external lock returns (uint256 liquidity) {
    ...
    uint256 balance0 = ILatteSwapBEP20(token0).balanceOf(address(this));
    uint256 balance1 = ILatteSwapBEP20(token1).balanceOf(address(this));
  ...
  function burn(address to) external lock returns (uint256 amount0, uint256 amount1) {
    ...
    uint256 balance0 = ILatteSwapBEP20(_token0).balanceOf(address(this));
    uint256 balance1 = ILatteSwapBEP20(_token1).balanceOf(address(this));
  ...
  function sync() external lock {
    _update(
      ILatteSwapBEP20(token0).balanceOf(address(this)),
      ILatteSwapBEP20(token1).balanceOf(address(this)),
      reserve0,
      reserve1
    );
  }
