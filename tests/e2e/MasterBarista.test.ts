...
describe("MasterBarista e2e", () => {
  ...
  beforeEach(async() => {
    ;({latteToken, beanBag, masterBarista, stakingTokens, LATTE_START_BLOCK, LATTE_PER_BLOCK, LATTE_BONUS_LOCK_UP_BPS} = await waffle.loadFixture(masterBaristaE2ETestFixture))
  ...
  context('when using pool', async() => {
    it('should distribute rewards according to the alloc point', async() => {
      ...
      await masterBaristaAsAlice.deposit(await alice.getAddress(), stakingTokens[0].address, ethers.utils.parseEther('100'));
      ...
      const latteEarnedFromStoken0 = await masterBarista.pendingLatte(stakingTokens[0].address, await alice.getAddress())
      assertAlmostEqual(
        LATTE_PER_BLOCK.mul(6000).div(10000).mul(3).toString(),
        latteEarnedFromStoken0.toString()
      )
      const latteEarnedFromLatte = await masterBarista.pendingLatte(latteToken.address, await alice.getAddress())
      assertAlmostEqual(
        LATTE_PER_BLOCK.mul(4000).div(10000).mul(1).toString(),
        latteEarnedFromLatte.toString()
      )
      ...
      assertAlmostEqual(
        LATTE_PER_BLOCK.mul(6000).div(10000).mul(4)
          .add(LATTE_PER_BLOCK.mul(4000).div(10000).mul(3)).toString(),
    ...
    it('should work', async() => {
      ...
      expect(
        await masterBarista.pendingLatte(latteToken.address, (await alice.getAddress())))
          .to.be.eq(LATTE_PER_BLOCK);
      ...
      expect(await masterBarista.pendingLatte(latteToken.address, (await alice.getAddress())))
        .to.be.eq(LATTE_PER_BLOCK.mul(2));
      ...
      await masterBaristaAsAlice["harvest(address,address)"](await alice.getAddress(), latteToken.address);
      expect(await latteToken.balanceOf(await alice.getAddress()))
        .to.be.eq(aliceLatteBefore.add(LATTE_PER_BLOCK.mul(3)));
      expect(await latteToken.balanceOf(await dev.getAddress()))
        .to.be.eq(LATTE_PER_BLOCK.mul(3).mul(1500).div(10000));
      ...
      expect(await masterBarista.pendingLatte(latteToken.address, await alice.getAddress()))
        .to.be.eq(LATTE_PER_BLOCK.mul(2));
      expect(await latteToken.balanceOf(await dev.getAddress()))
        .to.be.eq(LATTE_PER_BLOCK.mul(5).mul(1500).div(10000));
      ...
      expect(await masterBarista.pendingLatte(latteToken.address, await alice.getAddress()))
        .to.be.eq(LATTE_PER_BLOCK.mul(2).add(LATTE_PER_BLOCK.mul(5000).div(10000)));
      expect(await masterBarista.pendingLatte(latteToken.address, await bob.getAddress()))
        .to.be.eq(LATTE_PER_BLOCK.mul(5000).div(10000));
      expect(await latteToken.balanceOf(await dev.getAddress()))
        .to.be.eq(LATTE_PER_BLOCK.mul(6).mul(1500).div(10000));
      ...
      expect(await masterBarista.pendingLatte(latteToken.address, await alice.getAddress()))
        .to.be.eq(LATTE_PER_BLOCK.mul(2).add(LATTE_PER_BLOCK.mul(5000).div(10000)).add(LATTE_PER_BLOCK.mul(5000).div(10000)));
      expect(await masterBarista.pendingLatte(latteToken.address, await bob.getAddress()))
        .to.be.eq(LATTE_PER_BLOCK.mul(5000).div(10000).add(LATTE_PER_BLOCK.mul(5000).div(10000)));
      expect(await latteToken.balanceOf((await dev.getAddress())))
        .to.be.eq(LATTE_PER_BLOCK.mul(7).mul(1500).div(10000));
      ...
      expect(await masterBarista.pendingLatte(latteToken.address, await bob.getAddress()))
        .to.be.eq('0');
      expect(await latteToken.balanceOf(await bob.getAddress()))
        .to.be.eq(LATTE_PER_BLOCK.mul(2).mul(5000).div(10000).add(LATTE_PER_BLOCK.mul(5000).div(10000)));
      expect(await latteToken.balanceOf(await dev.getAddress()))
        .to.be.eq(LATTE_PER_BLOCK.mul(8).mul(1500).div(10000));
      ...
      expect(await latteToken.balanceOf(await alice.getAddress()))
        .to.be.eq(
          aliceLatteBefore.sub(ethers.utils.parseEther('300'))
          .add(LATTE_PER_BLOCK.mul(2))
          .add(LATTE_PER_BLOCK.mul(3).mul(5000).div(10000)
          .add(LATTE_PER_BLOCK.mul(2).mul(5000).div(10000))));
      expect(await masterBarista.pendingLatte(latteToken.address, await alice.getAddress()))
        .to.be.eq('0')
      expect(await masterBarista.pendingLatte(latteToken.address, await bob.getAddress()))
        .to.be.eq(LATTE_PER_BLOCK.mul(2).mul(5000).div(10000));
      expect(await latteToken.balanceOf(await bob.getAddress()))
        .to.be.eq(bobLatteBefore);
      expect(await latteToken.balanceOf(await dev.getAddress()))
        .to.be.eq(LATTE_PER_BLOCK.mul(10).mul(1500).div(10000))
      ...
      bobLatteBefore = await latteToken.balanceOf(await bob.getAddress())
      expect(await masterBarista.pendingLatte(latteToken.address, await alice.getAddress()))
        .to.be.eq(LATTE_PER_BLOCK.mul(8000).div(10000))
      expect(await latteToken.balanceOf(await alice.getAddress()))
        .to.be.eq(aliceLatteBefore);
      expect(await masterBarista.pendingLatte(latteToken.address, await bob.getAddress()))
        .to.be.eq(LATTE_PER_BLOCK.mul(2).mul(5000).div(10000).add(LATTE_PER_BLOCK.mul(2000).div(10000)))
      expect(await latteToken.balanceOf(await bob.getAddress()))
        .to.be.eq(bobLatteBefore);
      expect(await latteToken.balanceOf(await dev.getAddress()))
        .to.be.eq(LATTE_PER_BLOCK.mul(11).mul(1500).div(10000))
      ...
      expect(await latteToken.balanceOf(await dev.getAddress()))
        .to.be.eq(
          LATTE_PER_BLOCK.mul(11).mul(1500).div(10000)
          .add(LATTE_PER_BLOCK.mul(10).mul(4).mul(1500).div(10000).mul(3000).div(10000))
          .add(LATTE_PER_BLOCK.mul(4).mul(1500).div(10000))
        )
      expect(await latteToken.lockOf((await dev.getAddress())))
        .to.be.eq(LATTE_PER_BLOCK.mul(10).mul(4).mul(1500).div(10000).mul(7000).div(10000));
    });
  });
});