...
import { LATTE__factory, LATTE, BeanBag__factory, MasterBarista__factory, SimpleToken__factory, BeanBag, MasterBarista, SimpleToken, MockStakeTokenCallerContract__factory, MockStakeTokenCallerContract} from "../../../typechain";
...
export async function masterBaristaUnitTestFixture(maybeWallets?: Wallet[], maybeProvider?: MockProvider): Promise<IMasterBaristaUnitTestFixtureDTO> {
    ...
    const MockStakeTokenCallerContract = await ethers.getContractFactory("MockStakeTokenCallerContract", deployer) as MockStakeTokenCallerContract__factory
    const mockStakeTokenCaller = await MockStakeTokenCallerContract.deploy(latteToken.address, stakingTokens[0].address, masterBarista.address)
...
export async function masterBaristaE2ETestFixture(maybeWallets?: Wallet[], maybeProvider?: MockProvider): Promise<IMasterBaristaE2ETestFixtureDTO> {
  const LATTE_START_BLOCK = 5;
  const LATTE_PER_BLOCK = ethers.utils.parseEther('10');
  const LATTE_BONUS_LOCK_UP_BPS = 7000;
  const [deployer, bob, alice, dev] = await ethers.getSigners()
