...
import { BeanBag, BeanBag__factory, Booster, Booster__factory, LATTE, LATTE__factory, SimpleToken, SimpleToken__factory, MockWBNB, MockWBNB__factory, WNativeRelayer__factory, WNativeRelayer, WBNB__factory, WBNB } from "../../../typechain";
...
export async function boosterUnitTestFixture(maybeWallets?: Wallet[], maybeProvider?: MockProvider): Promise<IBoosterUnitTestFixtureDTO> {
  ...
  const MockERC721 = await smoddit(
      "MockERC721",
      deployer
  );
  ...
  const booster = await upgrades.deployProxy(Booster, [latteToken.address, mockMasterBarista.address, mockBoosterConfig.address, wNativeRelayer.address, wbnb.address]) as Booster
