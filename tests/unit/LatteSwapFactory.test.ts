...
import { LatteSwapFactory, LatteSwapFactory__factory, LatteSwapPair__factory, SimpleToken, SimpleToken__factory, WBNB, WBNB__factory } from '../../typechain'
...
describe('LatteSwapFactory', () => {
  ...
  context('createPair', () => {
    it('should create a pair', async() => {
      const create2Address = getCreate2Address(latteSwapFactory.address, [token0.address, token1.address], LatteSwapPairAbi.bytecode)
      const [sortedToken0, sortedToken1] = sortsBefore(token0.address, token1.address)  ? [token0.address, token1.address] : [token1.address, token0.address]
    ...
    it('should revert if the given pair is existed', async() => {
      const create2Address = getCreate2Address(latteSwapFactory.address, [token0.address, token1.address], LatteSwapPairAbi.bytecode)
      const [sortedToken0, sortedToken1] = token0.address.toLowerCase() < token1.address.toLowerCase() ? [token0.address, token1.address] : [token1.address, token0.address]
      await expect(latteSwapFactory.createPair(token0.address, token1.address))
        .to.emit(latteSwapFactory, 'PairCreated')
        .withArgs(sortedToken0, sortedToken1, create2Address, 1)

      await expect(latteSwapFactory.createPair(sortedToken0, sortedToken1))
        .to.be.revertedWith('LatteSwapFactory::createPair::PAIR_EXISTS')
      await expect(latteSwapFactory.createPair(sortedToken1, sortedToken0))
        .to.be.revertedWith('LatteSwapFactory::createPair::PAIR_EXISTS')
    })
  })
  ...
  context('setFeeTo', () => {
    ...
    it('should revert if non-feeToSetter try to set feeTo', async() => {
      ...
      await expect(latteSwapFactoryAsAlice.setFeeTo(await deployer.getAddress()))
        .to.be.revertedWith('LatteSwapFactory::setFeeTo::FORBIDDEN')
    })
  })
  ...
  context('setFeeToSetter', () => {
    ...
    it('should revert if non-feeToSetter try to set FeeToSetter', async() => {
      const latteSwapFactoryAsAlice = LatteSwapFactory__factory.connect(latteSwapFactory.address, alice)
      await expect(latteSwapFactoryAsAlice.setFeeToSetter(await deployer.getAddress()))
        .to.be.revertedWith('LatteSwapFactory::setFeeToSetter::FORBIDDEN')
    })
  })
})