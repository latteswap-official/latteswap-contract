...
describe('LatteNFT', () => {
  ...
  describe("#categoryToLatteNFTList()", () => {
    it('should return a list of tokenIds', async () => {
      ...
      expect((await latteNFT.categoryToLatteNFTList(0)).map((tokenId: BigNumber) => {
        return tokenId.toNumber()
      })).to.deep.eq([0,1,2,3,4,6,7,8,9,10])

      expect((await latteNFT.categoryToLatteNFTList(1)).map((tokenId: BigNumber) => {
        return tokenId.toNumber()
      })).to.deep.eq([5])

      expect((await latteNFT.categoryToLatteNFTList(2)).map((tokenId: BigNumber) => {
        return tokenId.toNumber()
      })).to.deep.eq([])
    })
  })
  ...
  describe("#categoryURI()", () => {
    context('when category is yet to be existed', () => {
      it('should revert', async () => {
        await expect(latteNFT.categoryURI(99)).to.revertedWith('LatteNFT::onlyExistingCategoryId::categoryId not existed')
      })
    })
  ...
  describe('#addCategoryInfo()', () => {
    context('when caller is not owner', async() => {
      it('should reverted', async() => {
          await expect(latteNFTAsBob.addCategoryInfo("NewCatagoryInfo", "/foo/bar"))
  ...
  describe('#updateCategoryInfo()', () => {
    context('when category is yet to be existed', () => {
      it('should revert', async () => {
        await expect(latteNFT.updateCategoryInfo(99, "updatedCategoryName", "/foo/bar"))
            .to.be.revertedWith('LatteNFT::onlyExistingCategoryId::categoryId not existed')
      })
    })
    context('when caller is not owner', async() => {
      it('should reverted', async() =>  {
        await expect(latteNFTAsBob.updateCategoryInfo(0, "updatedCategoryName", "/foo/bar"))
            .to.be.revertedWith("LatteNFT::onlyGovernance::only GOVERNANCE role")
      })
    })
  ...
  describe('#updateTokenCategory()', () => {
    context('when category is yet to be existed', () => {
      it('should revert', async () => {
        await expect(latteNFT.updateTokenCategory(0, 99))
            .to.be.revertedWith('LatteNFT::onlyExistingCategoryId::categoryId not existed')
      })
    })
    context('when caller is not owner', async() => {
      it('should reverted', async () => {
        await expect(latteNFTAsBob.updateTokenCategory(0, 0))
            .to.be.revertedWith("LatteNFT::onlyGovernance::only GOVERNANCE role")
      })
    })
  ...
  describe('#mint()', () => {
    context('when category is yet to be existed', () => {
      it('should revert', async () => {
        await expect(latteNFT.mint(await eve.getAddress(), 99, "tokenUrl"))
            .to.be.revertedWith('LatteNFT::onlyExistingCategoryId::categoryId not existed')
      })
    })
    context('when caller is not owner', async() => {
      it('should reverted', async() => {
        await expect(latteNFTAsBob.mint(await eve.getAddress(), 1, "tokenUrl"))
    ...
    context('when caller used to be minter', async() => {
      it('should reverted', async() => {
        ...
        await expect(latteNFT.mint(await eve.getAddress(), 1, "tokenUrl"))
            .to.be.revertedWith("LatteNFT::onlyMinter::only MINTER role")
      })
    })
    ...
    context('when paused', async () => {
      it('should reverted', async() => {
        ...
        await expect(latteNFT.mint(await eve.getAddress(), 0, "tokenUrl"))
  ...
  describe('#mintBatch()', () => {
    context('when category is yet to be existed', () => {
      it('should revert', async () => {
        await expect(latteNFT.mintBatch(await eve.getAddress(), 99, "tokenURI", 100))
            .to.be.revertedWith('LatteNFT::onlyExistingCategoryId::categoryId not existed')
      })
    })
    context('when caller is not owner', async() => {
      it('should reverted', async() => {
        await expect(latteNFTAsBob.mintBatch(await eve.getAddress(), 0, "tokenURI", 100))
            .to.be.revertedWith("LatteNFT::onlyMinter::only MINTER role")
      })
    })
    ...
    context('when caller used to be minter', async() => {
      it('should reverted', async() => {
        await latteNFT.revokeRole(await latteNFT.MINTER_ROLE(), await deployer.getAddress())
        await expect(latteNFT.mintBatch(await eve.getAddress(), 0, "tokenURI", 100))
            .to.be.revertedWith("LatteNFT::onlyMinter::only MINTER role")
      })
    })
    ...
    context('when size is zero', async() => {
      it('should reverted', async() => {
        await expect(latteNFT.mintBatch(await eve.getAddress(), 0, "tokenURI", 0))
    ...
    context('when mint batch', async() => {
      it('should mint batch', async() => {
        ...
        expect((await latteNFT.categoryToLatteNFTList(0)).reduce((accum: boolean, tokenId: BigNumber, index: number) => {
  ...
  describe('#setLatteName()', () => {
    context('when caller is not a governance', async() => {
      it('should reverted', async() => {
        await expect(latteNFTAsBob.setLatteName(0, "settedName"))
            .to.be.revertedWith("LatteNFT::onlyGovernance::only GOVERNANCE role")
      })
    })
    ...
    context('when caller used to be governance', async() => {
      it('should reverted', async() => {
        ...
        await expect(latteNFT.setLatteName(0, "settedName"))
            .to.be.revertedWith("LatteNFT::onlyGovernance::only GOVERNANCE role")
      })
    })
    ...
    context('when set latte name', async() => {
      it('should setted latte name', async() => {
        ...
        expect(await latteNFT.latteNames(0))
            .to.be.eq("settedName")
      })
    })
  })
  ...
  describe('#burn()', () => {
    context('when caller is not owner', async() => {
      it('should reverted', async() => {
        await expect(latteNFTAsBob.burn(0))
            .to.be.revertedWith("LatteNFT::onlyMinter::only MINTER role")
      })
    })
