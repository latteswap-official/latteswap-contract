...
import { DripBar, DripBar__factory, MockERC20, MockERC20__factory } from "../../typechain";
...
let dripBarAsDeployer: DripBar;
let dripBarAsAlice: DripBar;
let dripBarAsBob: DripBar;
let dripBarAsCat: DripBar;
...
describe("DripBar", () => {
  async function fixture() {
    ...
    const DripBar = (await ethers.getContractFactory("DripBar", deployer)) as DripBar__factory;
    dripBar = (await upgrades.deployProxy(DripBar, [await deployer.getAddress()])) as DripBar;
    ...
    await rewardTokenAsDeployer.approve(dripBar.address, constants.MaxUint256);
    await rewardToken2AsDeployer.approve(dripBar.address, constants.MaxUint256);
    mockedBlock = await TimeHelpers.latestBlockNumber();
    await TimeHelpers.advanceBlockTo(mockedBlock.add(1).toNumber());

    dripBarAsDeployer = DripBar__factory.connect(dripBar.address, deployer);
    dripBarAsAlice = DripBar__factory.connect(dripBar.address, alice);
    dripBarAsBob = DripBar__factory.connect(dripBar.address, bob);
    dripBarAsCat = DripBar__factory.connect(dripBar.address, cat);
  ...
  describe("#setRewardHolder", async () => {
    context("When the caller is not the owner", async () => {
      it("should be reverted", async () => {
        ...
        await expect(dripBarAsAlice.setRewardHolder(aliceAddr)).to.reverted;
      });
    });
    context("When the caller is the owner", async () => {
      it("should successfully change a reward holder", async () => {
        const aliceAddr = await alice.getAddress();
        await dripBarAsDeployer.setRewardHolder(aliceAddr);
        const holder = await dripBarAsDeployer.rewardHolder();
  ...
  describe("#currentEndBlock()", async () => {
    context("reward info is not existed yet", async () => {
      it("should return 0 as a current end block", async () => {
        ...
        const currentEndBlock = await dripBarAsDeployer.currentEndBlock(0);
    ...
    context("reward info is existed", async () => {
      it("should return a current reward info endblock as a current end block", async () => {
        ...
        await dripBarAsDeployer.addCampaignInfo(
          stakingToken.address,
          rewardToken.address,
          mockedBlock.add(8).toString()
        );
        await dripBarAsDeployer.addRewardInfo(0, mockedBlock.add(9).toString(), INITIAL_BONUS_REWARD_PER_BLOCK);
        let currentEndBlock = await dripBarAsDeployer.currentEndBlock(0);
        expect(currentEndBlock).to.eq(mockedBlock.add(9));
        TimeHelpers.advanceBlockTo(mockedBlock.add(9).toNumber());

        await dripBarAsDeployer.addRewardInfo(0, mockedBlock.add(10).toString(), INITIAL_BONUS_REWARD_PER_BLOCK);
        currentEndBlock = await dripBarAsDeployer.currentEndBlock(0);
  ...
  describe("#currentRewardPerBlock()", async () => {
    context("reward info is not existed yet", async () => {
      it("should return 0 as a current reward per block", async () => {
        const currentEndBlock = await dripBarAsDeployer.currentRewardPerBlock(0);
        expect(currentEndBlock).to.eq(BigNumber.from(0));
      });
    });
    context("reward info is existed", async () => {
      it("should return a current reward info endblock as a current reward per block", async () => {
        ...
        await dripBarAsDeployer.addRewardInfo(0, mockedBlock.add(9).toString(), INITIAL_BONUS_REWARD_PER_BLOCK);
        let currentRewardPerBlock = await dripBarAsDeployer.currentRewardPerBlock(0);
        ...
        await dripBarAsDeployer.addRewardInfo(
          0,
        ...
        currentRewardPerBlock = await dripBarAsDeployer.currentRewardPerBlock(0);
  ...
  describe("#addCampaignInfo", async () => {
    it("should return a correct campaign info length", async () => {
      let length = await dripBarAsDeployer.campaignInfoLen();
      expect(length).to.eq(0);
      await dripBarAsDeployer.addCampaignInfo(stakingToken.address, rewardToken.address, mockedBlock.add(9).toString());
      length = await dripBarAsDeployer.campaignInfoLen();
    ...
    context("When all parameters are valid", async () => {
      context("When the reward info is still within the limit", async () => {
        it("should still be able to push the new reward info with the latest as the newly pushed reward info", async () => {
          ...
          await dripBarAsDeployer.setRewardInfoLimit(2);
          let length = await dripBarAsDeployer.rewardInfoLen(0);
          expect(length).to.eq(0);
          // add the first reward info
          await dripBarAsDeployer.addRewardInfo(0, mockedBlock.add(11).toString(), INITIAL_BONUS_REWARD_PER_BLOCK);
          length = await dripBarAsDeployer.rewardInfoLen(0);
          expect(length).to.eq(1);
          ...
          const rewardInfo = await dripBarAsDeployer.campaignRewardInfo(0, 1);
          length = await dripBarAsDeployer.rewardInfoLen(0);
    ...
    context("When some parameters are invalid", async () => {
      context("When the caller isn't the owner", async () => {
        it("should reverted since there is a modifier only owner validating the ownership", async () => {
          ...
          // set reward info limit to 1
          await expect(dripBarAsAlice.setRewardInfoLimit(1)).to.be.reverted;
          await expect(dripBarAsAlice.addRewardInfo(0, mockedBlock.add(11).toString(), INITIAL_BONUS_REWARD_PER_BLOCK))
            .to.be.reverted;
        });
      });
      context("When reward info exceed the limit", async () => {
        it("should reverted since the length of reward info exceed the limit", async () => {
          ...
          // set reward info limit to 1
          await dripBarAsDeployer.setRewardInfoLimit(1);
          // add the first reward info
          await dripBarAsDeployer.addRewardInfo(0, mockedBlock.add(11).toString(), INITIAL_BONUS_REWARD_PER_BLOCK);
          await expect(
            dripBarAsDeployer.addRewardInfo(0, mockedBlock.add(11).toString(), INITIAL_BONUS_REWARD_PER_BLOCK)
          ).to.be.revertedWith("DripBar::addRewardInfo::reward info length exceeds the limit");
        });
      });
      context("When the newly added reward info endblock is less that the start block", async () => {
        it("should be reverted", async () => {
          ...
          // add the first reward info
          await expect(
            dripBarAsDeployer.addRewardInfo(0, mockedBlock.sub(1).toString(), INITIAL_BONUS_REWARD_PER_BLOCK)
          ).to.reverted;
        });
      });
      context("When newly added reward info endblock is less than current end block", async () => {
        it("should reverted with the message DripBar::addRewardInfo::bad new endblock", async () => {
          const mintedReward = INITIAL_BONUS_REWARD_PER_BLOCK.mul(mockedBlock.add(11).sub(mockedBlock.add(8)));
          await rewardTokenAsDeployer.mint(await deployer.getAddress(), mintedReward);
          await dripBarAsDeployer.addCampaignInfo(
            stakingToken.address,
          ...
          await dripBarAsDeployer.addRewardInfo(0, mockedBlock.add(11).toString(), INITIAL_BONUS_REWARD_PER_BLOCK);
          await expect(
            dripBarAsDeployer.addRewardInfo(0, mockedBlock.add(1).toString(), INITIAL_BONUS_REWARD_PER_BLOCK)
          ).to.be.revertedWith("DripBar::addRewardInfo::bad new endblock");
        });
      });
      context("When the current reward period has ended", async () => {
        it("should reverted with the message DripBar::addRewardInfo::reward period ended", async () => {
          ...
          await expect(
            dripBarAsDeployer.addRewardInfo(0, mockedBlock.add(12).toString(), INITIAL_BONUS_REWARD_PER_BLOCK)
          ).to.be.revertedWith("DripBar::addRewardInfo::reward period ended");
        });
      });
    });
  });
    ...
    context("With invalid parameters", async () => {
      context("When there is NO predefined campaign", async () => {
        it("should revert the tx since an array of predefined campaigns is out of bound", async () => {
          ...
          await stakingTokenAsAlice.approve(dripBar.address, ethers.utils.parseEther("100"));

          // alice deposit @block number #(mockedBlock+10)
          await expect(dripBarAsAlice.deposit(BigNumber.from(0), ethers.utils.parseEther("100"))).to.be.reverted;
        });
      });
    ...
    context("With valid parameters", async () => {
          ...
          context("When there is only one beneficial who get the reward (alice)", async () => {
            context("When alice's deposit block is is in the middle of start and end block", async () => {
              context("When alice deposit again with different block time", async () => {
                it("should return reward from previous deposit to alice", async () => {
                  ...
                  // alice & bob approve grazing range
                  await stakingTokenAsAlice.approve(dripBar.address, ethers.utils.parseEther("300"));

                  // alice deposit @block number #(mockedBlock+7)
                  await dripBarAsAlice.deposit(BigNumber.from(0), ethers.utils.parseEther("100"));
                  // bob deposit @block number #(mockedBlock+8)
                  await dripBarAsAlice.deposit(BigNumber.from(0), ethers.utils.parseEther("200"));
                  ...
                  expect((await dripBarAsAlice.campaignInfo(0)).lastRewardBlock).to.eq(currentBlockNum);
                  expect((await dripBarAsAlice.campaignInfo(0)).accRewardPerShare).to.eq(expectedAccRewardPerShare);
                  expect((await dripBarAsAlice.campaignInfo(0)).totalStaked).to.eq(ethers.utils.parseEther("300"));
                  ...
                  Assert.assertAlmostEqual(
                    (await dripBarAsAlice.pendingReward(BigNumber.from(0), await alice.getAddress())).toString(),
              ...
              context("when calling update campaign within the range of reward blocks", async () => {
                context("when the current block time (alice time) is before the starting time", async () => {
                  it("#pendingReward() will recalculate the accuReward and return the correct reward corresponding to the starting blocktime", async () => {
                    ...
                    await dripBarAsAlice.updateCampaign(BigNumber.from(0));
                    const currentBlockNum = await TimeHelpers.latestBlockNumber();
                    ...
                    expect(await dripBarAsAlice.pendingReward(BigNumber.from(0), await alice.getAddress())).to.eq(
              ...
              context("when calling update campaign out of the range of reward blocks", async () => {
                it("should update a correct reward per share, pending rewards", async () => {
                  ...
                  expect((await dripBarAsAlice.campaignInfo(0)).lastRewardBlock).to.eq(mockedBlock.add(8));
                  expect((await dripBarAsAlice.campaignInfo(0)).accRewardPerShare).to.eq(expectedAccRewardPerShare);
                  expect(await dripBarAsAlice.pendingReward(BigNumber.from(0), await alice.getAddress())).to.eq(
                    ethers.utils.parseEther("200")
                  );
            ...
            context("When a deposit block exceeds the end block", async () => {
              it("won't distribute any rewards to alice", async () => {
                ...
                await dripBarAsDeployer.addRewardInfo(0, mockedBlock.add(8).toString(), INITIAL_BONUS_REWARD_PER_BLOCK);
                // mint staking token to alice
                await stakingTokenAsDeployer.mint(await alice.getAddress(), ethers.utils.parseEther("100"));
                // alice approve grazing range
                await stakingTokenAsAlice.approve(dripBar.address, ethers.utils.parseEther("100"));
                ...
                expect((await dripBarAsAlice.campaignInfo(0)).lastRewardBlock).to.eq(
            ...
            context("When alice and bob deposit within the range of reward blocks", async () => {
              context("when calling update campaign within the range of reward blocks", async () => {
                it("should update a correct reward per share and pending rewards", async () => {
                  ...
                  // alice & bob approve grazing range
                  await stakingTokenAsAlice.approve(dripBar.address, ethers.utils.parseEther("100"));
                  await stakingTokenAsBob.approve(dripBar.address, ethers.utils.parseEther("100"));
                  // alice deposit @block number #(mockedBlock+9)
                  await dripBarAsAlice.deposit(BigNumber.from(0), ethers.utils.parseEther("100"));
                  // bob deposit @block number #(mockedBlock+10)
                  await dripBarAsBob.deposit(BigNumber.from(0), ethers.utils.parseEther("100"));
                  ...
                  expect(await dripBarAsBob.pendingReward(BigNumber.from(0), await bob.getAddress())).to.eq(
        ...
        context("When there are multiple reward info (multiple phases)", async () => {
          context("When bob finish deposit within the first phase", async () => {
            it("should accrue the correct reward corresponding to different phases", async () => {
              ...
              await dripBarAsDeployer.addRewardInfo(0, mockedBlock.add(12).toString(), INITIAL_BONUS_REWARD_PER_BLOCK);
              ...
              expect(await dripBarAsAlice.pendingReward(BigNumber.from(0), await bob.getAddress())).to.eq(
      ...
      context("When there are multiple campaigns", async () => {
        it("should correctly separate rewards and total staked", async () => {
          ...
          await dripBarAsDeployer.addRewardInfo(0, mockedBlock.add(13).toString(), INITIAL_BONUS_REWARD_PER_BLOCK);
          ...
          // alice & bob approve grazing range
          await stakingTokenAsAlice.approve(dripBar.address, ethers.utils.parseEther("1000"));
          await stakingTokenAsBob.approve(dripBar.address, ethers.utils.parseEther("1000"));

          // ### campaign 0 ###
          // alice deposit @block number #(mockedBlock+9)
          await dripBarAsAlice.deposit(BigNumber.from(0), ethers.utils.parseEther("100"));
          // bob deposit @block number #(mockedBlock+10)
          await dripBarAsBob.deposit(BigNumber.from(0), ethers.utils.parseEther("200"));
          ...
          await dripBarAsAlice.deposit(BigNumber.from(1), ethers.utils.parseEther("400"));
          // bob deposit @block number #(mockedBlock+16)
          await dripBarAsBob.deposit(BigNumber.from(1), ethers.utils.parseEther("600"));
          ...
          expect((await dripBarAsAlice.campaignInfo(1)).lastRewardBlock).to.eq(currentBlockNum);
          expect((await dripBarAsAlice.campaignInfo(1)).accRewardPerShare).to.eq(expectedAccRewardPerShare);
          expect((await dripBarAsAlice.campaignInfo(1)).totalStaked).to.eq(ethers.utils.parseEther("1000"));
          ...
          expect(await dripBarAsAlice.pendingReward(BigNumber.from(1), await alice.getAddress())).to.eq(
          ...
          expect(await dripBarAsAlice.pendingReward(BigNumber.from(1), await bob.getAddress())).to.eq(
  ...
  describe("#emergencyWithdraw()", async () => {
    it("should return the correct deposit amount to the user", async () => {
      ...
      await dripBarAsDeployer.addCampaignInfo(stakingToken.address, rewardToken.address, mockedBlock.add(5).toString());
      ...
      await dripBarAsAlice.emergencyWithdraw(BigNumber.from(0));
      expect(await (await dripBarAsAlice.campaignInfo(0)).totalStaked).to.eq(0);
    ...
    it("should reset all user's info", async () => {
      ...
      // alice deposit @block number #(mockedBlock+9)
      await dripBarAsAlice.deposit(BigNumber.from(0), ethers.utils.parseEther("100"));
      let userInfo = await dripBarAsAlice.userInfo(BigNumber.from(0), await alice.getAddress());
      expect(await stakingToken.balanceOf(await alice.getAddress())).to.eq(BigNumber.from(0));
      expect(userInfo.amount).to.eq(ethers.utils.parseEther("100"));
      expect(userInfo.rewardDebt).to.eq(BigNumber.from(0));
      // alice withdraw from the campaign
      await dripBarAsAlice.emergencyWithdraw(BigNumber.from(0));
      userInfo = await dripBarAsAlice.userInfo(BigNumber.from(0), await alice.getAddress());
  ...
  describe("#emergencyRewardWithdraw()", async () => {
    context("When the caller is not the owner", async () => {
      it("should revert", async () => {
        ...
        await rewardToken2AsDeployer.mint(dripBar.address, ethers.utils.parseEther("1000"));
        await dripBarAsDeployer.addCampaignInfo(
          stakingToken.address,
          rewardToken.address,
        ...
        await expect(
          dripBarAsAlice.emergencyRewardWithdraw(
            BigNumber.from(0),
        ...
        context("When there is a multiple campaign sharing the same reward token", async () => {
          context("When totalStaked is 0", () => {
            it("should be reverted", async () => {
              ...
              await dripBarAsDeployer.addRewardInfo(0, mockedBlock.add(10).toString(), INITIAL_BONUS_REWARD_PER_BLOCK);
              await dripBarAsDeployer.addRewardInfo(1, mockedBlock.add(20).toString(), INITIAL_BONUS_REWARD_PER_BLOCK);
              ...
              await expect(
                dripBarAsDeployer.emergencyRewardWithdraw(BigNumber.from(0), mintedReward, await deployer.getAddress())
              ).to.revertedWith("DripBar::emergencyRewardWithdraw::not enough reward token");
            });
          });
          ...
          context("when currentStakingPendingReward + withdraw amount > campaign's total rewards)", async () => {
            it("should be reverted", async () => {
              ...
              await stakingTokenAsAlice.approve(dripBar.address, ethers.utils.parseEther("400"));

              // alice deposit 400
              await dripBarAsAlice.deposit(BigNumber.from(0), ethers.utils.parseEther("400"));
              ...
              await expect(
                dripBarAsDeployer.emergencyRewardWithdraw(
                  BigNumber.from(0),
        ...
        context("When there is a multiple campaign sharing the same reward token", async () => {
          context("When totalStaked is 0", () => {
            it("should return desired reward token based on total reward token to the beneficiary", async () => {
              ...
              // emergency withdraw campaign 0
              await dripBarAsDeployer.emergencyRewardWithdraw(
                BigNumber.from(0),
                ethers.utils.parseEther("100"),
                await deployer.getAddress()
              );
              expect(await rewardToken.balanceOf(dripBar.address)).to.eq(
                mintedReward.sub(ethers.utils.parseEther("100"))
              );
              expect(await (await dripBarAsDeployer.campaignInfo(0)).totalRewards).to.eq(
                mintedReward1.sub(ethers.utils.parseEther("100"))
              );
              expect(await (await dripBarAsDeployer.campaignInfo(1)).totalRewards).to.eq(mintedReward2);
              ...
              expect(await (await dripBarAsDeployer.campaignInfo(1)).totalRewards).to.eq(
        ...
        context("When there is a multiple campaign with different reward tokens", async () => {
          context("When totalStaked is 0", () => {
            it("should return desired reward token based on total reward token to the beneficiary", async () => {
              ...
              expect(await rewardToken2.balanceOf(dripBar.address)).to.eq(
                mintedReward2.sub(ethers.utils.parseEther("1000"))
              );
              expect(await (await dripBarAsDeployer.campaignInfo(0)).totalRewards).to.eq(
                mintedReward1.sub(ethers.utils.parseEther("100"))
    ...
    context("With valid parameters", async () => {
            ...
            context("When alice's deposit block is in the middle of start and end block", async () => {
              context("when alice withdraw within the range of reward blocks", async () => {
                it("should receive a reward correctly", async () => {
                  ...
                  await dripBarAsAlice.withdraw(BigNumber.from(0), ethers.utils.parseEther("100"));

                  expect(await rewardToken.balanceOf(await alice.getAddress())).to.eq(ethers.utils.parseEther("200"));
                  expect(await (await dripBarAsAlice.campaignInfo(BigNumber.from(0))).totalStaked.toString()).to.eq(
        ...
        context("When there are multiple reward info (multiple phases)", async () => {
          context("When alice finish deposit within the first phase", async () => {
            it("should accrue the correct reward corresponding to different phases", async () => {
              ...
              await dripBarAsAlice.withdraw(BigNumber.from(0), ethers.utils.parseEther("100"));
              await dripBarAsBob.withdraw(BigNumber.from(0), ethers.utils.parseEther("100"));
      ...
      context("When there are multiple campaigns", async () => {
        it("should correctly separate rewards and total staked", async () => {
          ...
          await dripBarAsAlice.withdraw(BigNumber.from(0), ethers.utils.parseEther("100"));
          await dripBarAsBob.withdraw(BigNumber.from(0), ethers.utils.parseEther("200"));
          ...
          await dripBarAsAlice.withdraw(BigNumber.from(1), ethers.utils.parseEther("400"));
          await dripBarAsBob.withdraw(BigNumber.from(1), ethers.utils.parseEther("600"));
          // alice should expect to see her  reward according to calculated reward per share and her deposit

          expect(await (await dripBarAsAlice.campaignInfo(BigNumber.from(1))).totalStaked.toString()).to.eq(
            ethers.utils.parseEther("0")
          );
          expect(await (await dripBarAsBob.campaignInfo(BigNumber.from(1))).totalStaked.toString()).to.eq(
            ethers.utils.parseEther("0")
          );
    ...
    context("With valid parameters", async () => {
            ...
            context("When alice's deposit block is in the middle of start and end block", async () => {
              context("when alice harvest within the range of reward blocks", async () => {
                it("should receive a reward correctly", async () => {
                  ...
                  await dripBarAsAlice.deposit(BigNumber.from(0), ethers.utils.parseEther("100"));
                  // alice withdraw @block number #(mockedBlock+8)
                  await dripBarAsAlice.harvest([BigNumber.from(0)]);
        ...
        context("When there are multiple reward info (multiple phases)", async () => {
          context("When alice finish deposit within the first phase", async () => {
            it("should accrue the correct reward corresponding to different phases", async () => {
              ...
              await dripBarAsAlice.harvest([BigNumber.from(0)]);
              await dripBarAsBob.harvest([BigNumber.from(0)]);
      ...
      context("When there are multiple campaigns", async () => {
        it("should correctly separate rewards and total staked", async () => {
          ...
          expect((await dripBarAsAlice.campaignInfo(1)).totalStaked).to.eq(ethers.utils.parseEther("1000"));
          // harvest
          await dripBarAsAlice.harvest([BigNumber.from(1)]);
          await dripBarAsBob.harvest([BigNumber.from(1)]);
          // alice should expect to see her pending reward according to calculated reward per share and her deposit
