...
describe('BoosterConfig', () => {
    ...
    describe('#updateCurrentEnergy()', () => {
        ...
        context('when the caller is not allowance', async() => {
            it('should reverted', async () => {
                await expect(boosterConfigAsAlice.updateCurrentEnergy((await eve.getAddress()), 1, 2))
                    .to.be.revertedWith("BoosterConfig::onlyCaller::only eligible caller")
            });
        })
    })
    ...
    describe('#setStakeTokenAllowance()', () => {
        ...
        context('when caller is not owner', async() => {
            it('should reverted', async() => {
                await expect(boosterConfigAsAlice.setStakeTokenAllowance(await eve.getAddress(), true)).to.be.revertedWith("Ownable: caller is not the owner")
            })
        })

        context('when stake token is address zero', async() => {
            it('should reverted', async() => {
                await expect(boosterConfig.setStakeTokenAllowance(ethers.constants.AddressZero, true)).to.be.revertedWith("BoosterConfig::setStakeTokenAllowance::_stakeToken must not be address(0)")
            })
        })
    })
    ...
    describe('#setCallerAllowance()', () => {
        ...
        context('when caller is not owner', async() => {
            it('should reverted', async() => {
                await expect(boosterConfigAsBob.setCallerAllowance(await eve.getAddress(), true)).to.be.revertedWith("Ownable: caller is not the owner")
            })
        })

        context('when caller is address zero', async() => {
            it('should reverted', async() => {
                await expect(boosterConfig.setCallerAllowance(ethers.constants.AddressZero, true)).to.be.revertedWith("BoosterConfig::setCallerAllowance::_caller must not be address(0)")
            })
        })
    })
    ...
    describe('#setBatchBoosterNFTEnergyInfo()', () => {
        ...
        context('when caller is not owner', async() => {
            it('should revert', async() => {
                ...
                await expect(boosterConfigAsBob.setBatchBoosterNFTEnergyInfo([nft1, nft2])).to.be.revertedWith("Ownable: caller is not the owner")
            })
        })
    })
    ...
    describe('#setBoosterNFTEnergyInfo()', () => {
        ...
        context('when caller is not owner', async() => {
            it('should reverted', async() => {
                ...
                await expect(boosterConfigAsBob.setBoosterNFTEnergyInfo(nft)).to.be.revertedWith("Ownable: caller is not the owner")
            })
        })
    })
    ...
    describe('#setStakingTokenBoosterAllowance()', () => {
        context('when set staking token booster allowance', async() => {
            it('should set staking token booster allowance', async() => {
                ...
                const boosterAllowance = await boosterConfig.boosterNftAllowance(await alice.getAddress(), allowance[0].nftAddress, allowance[0].nftTokenId)
        ...
        context('when caller is not owner', async() => {
            it('should reverted', async() => {
                ...
                await expect(boosterConfigAsBob.setStakingTokenBoosterAllowance(boosterAllowanceParams)).to.be.revertedWith("Ownable: caller is not the owner")
            })
        })
        ...
        context('when staking token booster is not allowance', async() => {
            it('should reverted', async() => {
                ...
                await expect(boosterConfig.setStakingTokenBoosterAllowance(boosterAllowanceParams)).to.be.revertedWith("BoosterConfig::setStakingTokenBoosterAllowance:: bad staking token")
            })
        })
    })
}) 