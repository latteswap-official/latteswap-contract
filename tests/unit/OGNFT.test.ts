...
import { BeanBag, Booster, LATTE, MockMasterBarista, MockOGOwnerToken, MockWBNB, OGNFT, OGNFT__factory, OGOwnerToken, SimpleToken, WNativeRelayer } from "../../typechain";
...
describe('OGNFT', () => {
  ...
  beforeEach(async() => {
    ...
    ;({ ogNFT, wNativeRelayer, wbnb, stakingTokens, masterBarista, beanBag, latteToken, ogOwnerToken } = await waffle.loadFixture(ogNFTUnitTestFixture))
  ...
  describe("#categoryToLatteNFTList()", () => {
    it('should return a list of tokenIds', async () => {
      ...
      expect((await ogNFT.categoryToLatteNFTList(0)).map((tokenId: BigNumber) => {
        return tokenId.toNumber()
      })).to.deep.eq([0,1,2,3,4,6,7,8,9,10])

      expect((await ogNFT.categoryToLatteNFTList(1)).map((tokenId: BigNumber) => {
        return tokenId.toNumber()
      })).to.deep.eq([5])

      expect((await ogNFT.categoryToLatteNFTList(2)).map((tokenId: BigNumber) => {
        return tokenId.toNumber()
      })).to.deep.eq([])
    })
  })
  ...
  describe('#addCategoryInfo()', () => {
    context('when caller is not owner', async() => {
      it('should reverted', async() => {
          await expect(ogNFTAsBob.addCategoryInfo("NewCatagoryInfo", "/foo/bar"))
              .to.be.revertedWith("LatteNFT::onlyGovernance::only GOVERNANCE role")
      })
    })
  ...
  describe('#updateCategoryInfo()', () => {
    context('when category is yet to be existed', () => {
      it('should revert', async () => {
        await expect(ogNFT.updateCategoryInfo(99, "updatedCategoryName", "/foo/bar"))
            .to.be.revertedWith('LatteNFT::onlyExistingCategoryId::categoryId not existed')
      })
    })
    context('when caller is not owner', async() => {
      it('should reverted', async() =>  {
        await expect(ogNFTAsBob.updateCategoryInfo(0, "updatedCategoryName", "/foo/bar"))
            .to.be.revertedWith("LatteNFT::onlyGovernance::only GOVERNANCE role")
      })
    })
  ...
  describe('#updateTokenCategory()', () => {
    context('when category is yet to be existed', () => {
      it('should revert', async () => {
        await expect(ogNFT.updateTokenCategory(0, 99))
            .to.be.revertedWith('LatteNFT::onlyExistingCategoryId::categoryId not existed')
      })
    })
    context('when caller is not owner', async() => {
      it('should reverted', async () => {
        await expect(ogNFTAsBob.updateTokenCategory(0, 0))
            .to.be.revertedWith("LatteNFT::onlyGovernance::only GOVERNANCE role")
      })
    })
  ...
  describe('#mint()', () => {
    context('when category is yet to be existed', () => {
      it('should revert', async () => {
        await expect(ogNFT.mint(await alice.getAddress(), 99, "tokenUrl"))
            .to.be.revertedWith('LatteNFT::onlyExistingCategoryId::categoryId not existed')
      })
    })
    context('when caller is not owner', async() => {
      it('should reverted', async() => {
        await expect(ogNFTAsBob.mint(await alice.getAddress(), 1, "tokenUrl"))
            .to.be.revertedWith("LatteNFT::onlyMinter::only MINTER role")
      })
    })
    ...
    context('when caller used to be minter', async() => {
      it('should reverted', async() => {
        ...
        await expect(ogNFT.mint(await alice.getAddress(), 1, "tokenUrl"))
            .to.be.revertedWith("LatteNFT::onlyMinter::only MINTER role")
      })
    })
    ...
    context('when paused', async () => {
      it('should reverted', async() => {
        await ogNFT.pause()
        await expect(ogNFT.mint(await alice.getAddress(), 0, "tokenUrl"))
            .to.be.revertedWith("revert ERC721Pausable: token transfer while paused")
      })
    })
      ...
      context('without rewards to be harvest()', () => {
        context('when category 0', () => {
          it('should mint', async() => {
            ...
            expect(await ((ogOwnerToken as unknown) as MockOGOwnerToken).balanceOf(masterBarista.address)).to.eq(parseEther('1'))
  ...
  describe('#mintBatch()', () => {
    context('when category is yet to be existed', () => {
      it('should revert', async () => {
        await expect(ogNFT.mintBatch(await alice.getAddress(), 99, "tokenURI", 100))
            .to.be.revertedWith('LatteNFT::onlyExistingCategoryId::categoryId not existed')
      })
    })
    context('when caller is not owner', async() => {
      it('should reverted', async() => {
        await expect(ogNFTAsBob.mintBatch(await alice.getAddress(), 0, "tokenURI", 100))
            .to.be.revertedWith("LatteNFT::onlyMinter::only MINTER role")
      })
    })
    ...
    context('when caller used to be minter', async() => {
      it('should reverted', async() => {
        await ogNFT.revokeRole(await ogNFT.MINTER_ROLE(), await deployer.getAddress())
        await expect(ogNFT.mintBatch(await alice.getAddress(), 0, "tokenURI", 100))
            .to.be.revertedWith("LatteNFT::onlyMinter::only MINTER role")
      })
    })
    ...
    context('when size is zero', async() => {
      it('should reverted', async() => {
        await expect(ogNFT.mintBatch(await alice.getAddress(), 0, "tokenURI", 0))
            .to.be.revertedWith("LatteNFT::mintBatch::size must be granter than zero")
      })
    })
    ...
    context('when parameters are valid', async() => {
      context('without rewards to be harvested', () => {
        it('should mint batch', async() => {
          ...
          expect((await ogNFT.categoryToLatteNFTList(0)).reduce((accum: boolean, tokenId: BigNumber, index: number) => {
  ...
  describe('#setLatteName()', () => {
    context('when caller is not a governance', async() => {
      it('should reverted', async() => {
        await expect(ogNFTAsBob.setLatteName(0, "settedName"))
            .to.be.revertedWith("LatteNFT::onlyGovernance::only GOVERNANCE role")
      })
    })
    ...
    context('when caller used to be governance', async() => {
      it('should reverted', async() => {
        ...
        await expect(ogNFT.setLatteName(0, "settedName"))
            .to.be.revertedWith("LatteNFT::onlyGovernance::only GOVERNANCE role")
      })
    })
    ...
    context('when set latte name', async() => {
      it('should setted latte name', async() => {
        ...
        expect(await ogNFT.latteNames(0))
            .to.be.eq("settedName")
      })
    })
  })
  ...
  describe('#burn()', () => {
    context('when caller is not owner', async() => {
      it('should reverted', async() => {
        await expect(ogNFTAsBob.burn(0))
            .to.be.revertedWith("LatteNFT::onlyMinter::only MINTER role")
      })
    })
