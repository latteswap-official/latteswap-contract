...
describe("Booster", () => {
    ...
    context("when stake token and nft are allowed", () => {
      ...
      context("when there is no reward to harvest", () => {
        it("should successfully stake nft", async () => {
          ...
          expect((await booster.userStakingNFT(stakingTokens[0].address, deployerAddr)).nftAddress.toLowerCase()).to.eq(
  ...
  describe("#harvest()", () => {
    context("when sending multiple stake tokens", () => {
      it("should harvest multiple stake tokens", async () => {
        ...
        expect((await booster.userStakingNFT(stakingTokens[0].address, ownerAddress)).nftTokenId).to.eq(1);
        expect((await booster.userStakingNFT(stakingTokens[1].address, ownerAddress)).nftAddress.toLowerCase()).to.eq(
          nftToken.address.toLowerCase()
        );
    ...
    context("when the pool ain't the same as a reward", () => {
      context("when harvest an disallowed token", () => {
        it("should reverted", async () => {
          await expect(booster["harvest(address)"](stakingTokens[0].address)).to.revertedWith(
            "Booster::isStakeTokenOK::bad stake token"
          );
        });
      });
      ...
      context("without energy", () => {
        it("should emit a Harvest with 0 reward", async () => {
          ...
          await expect(booster["harvest(address)"](stakingTokens[0].address))
            .to.emit(booster, "Harvest")
    ...
    context("when the pool is the same as a reward", () => {
      context("when harvest an disallowed token", () => {
        it("should reverted", async () => {
          await expect(booster["harvest(address)"](latteToken.address)).to.revertedWith(
            "Booster::isStakeTokenOK::bad stake token"
          );
        });
      });
      ...
      context("without energy", () => {
        it("should emit a Harvest with 0 reward", async () => {
          ...
          const ownerAddress = await deployer.getAddress();
          await expect(booster["harvest(address)"](latteToken.address))
            .to.emit(booster, "Harvest")
            .withArgs(ownerAddress, latteToken.address, 0);
        });
      });
      ...
      context("with some energy", () => {
        it("should harvest the reward", async () => {
          ...
          await expect(boosterAsAlice["harvest(address)"](latteToken.address))
            .to.emit(booster, "Harvest")
            .withArgs(ownerAddress, latteToken.address, parseEther("100").add(parseEther("10")));
          // owner is expected to get 100 reward + 10 extra rewards from staking an nft
          expect(await latteToken.balanceOf(ownerAddress)).to.eq(parseEther("100").add(parseEther("10")));
          // dev is expected to get 15% of 10 extra reward, thus making the dev able to collect 1.5
          expect(await latteToken.balanceOf(await dev.getAddress())).to.eq(parseEther("1.5"));
  ...
  describe("#stake()", () => {
    context("when stake a disallowed token", () => {
      it("should revert", async () => {
        await expect(booster.stake(stakingTokens[0].address, parseEther("10"))).to.revertedWith(
          "Booster::isStakeTokenOK::bad stake token"
        );
      });
    });
        ...
        context("when msg.value != amount", () => {
          it("should revert", async () => {
            it("should be able to stake", async () => {
              ...
              await expect(
                boosterAsAlice.stake(wbnb.address, parseEther("100"), {
                  value: parseEther("99"),
                })
        ...
        context("when there is a reward to harvest", () => {
          context("with energy", () => {
            it("should stake a token with harvest a reward + some extra energy", async () => {
              ...
              await expect(
                boosterAsAlice.stake(wbnb.address, parseEther("50"), {
                  value: parseEther("50"),
                })
              )
                .to.emit(booster, "MasterBaristaCall")
                .withArgs(ownerAddress, parseEther("10"), wbnb.address, parseEther("10"), "0")
                .to.emit(booster, "Stake")
                .withArgs(ownerAddress, wbnb.address, parseEther("50"));
        ...
        context("when sending msg value with non-native stake token", () => {
          it("should revert", async () => {
            it("should be able to stake", async () => {
              ...
              await expect(
                boosterAsAlice.stake(stakingTokens[0].address, parseEther("100"), {
                  value: parseEther("100"),
                })
          ...
          context("with energy", () => {
            it("should stake a token with harvest a reward + some extra energy", async () => {
              ...
              await expect(boosterAsAlice.stake(stakingTokens[0].address, parseEther("50")))
                .to.emit(booster, "MasterBaristaCall")
                .withArgs(ownerAddress, parseEther("10"), stakingTokens[0].address, parseEther("10"), "0")
                .to.emit(booster, "Stake")
                .withArgs(ownerAddress, stakingTokens[0].address, parseEther("50"));
        ...
        context("with energy", () => {
          it("should stake a token with harvest a reward + some extra energy", async () => {
            ...
            await expect(boosterAsAlice.stake(latteToken.address, parseEther("50")))
              .to.emit(booster, "MasterBaristaCall")
              .withArgs(ownerAddress, parseEther("10"), latteToken.address, parseEther("10"), "0")
              .to.emit(booster, "Stake")
              .withArgs(ownerAddress, latteToken.address, parseEther("50"));
  ...
  describe("#unstake()", () => {
    context("when stake a disallowed token", () => {
      it("should revert", async () => {
        await expect(booster.unstake(stakingTokens[0].address, parseEther("10"))).to.revertedWith(
          "Booster::isStakeTokenOK::bad stake token"
        );
      });
    });
  ...
  describe("#emergencyWithdraw()", () => {
    context("when stake a disallowed token", () => {
      it("should revert", async () => {
        await expect(booster.emergencyWithdraw(stakingTokens[0].address)).to.revertedWith(
          "Booster::isStakeTokenOK::bad stake token"
        );
      });
    });
    it("should ble able to withdraw the staking token considerless the reward", async () => {
      ...
      await expect(boosterAsAlice.emergencyWithdraw(stakingTokens[0].address))
        .to.emit(booster, "EmergencyWithdraw")
        .withArgs(ownerAddress, stakingTokens[0].address, parseEther("10"));
      expect(await stakingTokens[0].balanceOf(ownerAddress)).to.eq(parseEther("10"));
    });
  });
