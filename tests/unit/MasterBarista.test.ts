...
describe("MasterBarista", () => {
  ...
  beforeEach(async () => {
    ;({latteToken, beanBag, masterBarista, stakingTokens, LATTE_START_BLOCK, LATTE_PER_BLOCK, LATTE_BONUS_LOCK_UP_BPS, mockStakeTokenCaller} = await waffle.loadFixture(masterBaristaUnitTestFixture))
  ...
  describe('#setpool()', () => {
    context('when adding a new pool', () => {
      it('should add new pool and update pool having a bps alloc point', async() => {
        ...
        for(let i = 0; i < stakingTokens.length; i++) {
          ...
          assertAlmostEqual(
            latteAllocPoint.mul(10000).div(totalAlloc).toString(),
            allocBps.toString()
          )
    ...
    context('when the stakeToken is already added to the pool', () => {
      it('should revert', async() => {
        ...
        await expect(masterBarista.addPool(stakingTokens[0].address, poolAlloc))
          .to.be.revertedWith("MasterBarista::addPool::_stakeToken duplicated")
      });
    })

    context('when the admin try to add address(0)', () => {
      it('should revert', async() => {
        await expect(masterBarista.addPool(ethers.constants.AddressZero, 1000))
          .to.be.revertedWith("MasterBarista::addPool::_stakeToken must not be address(0) or address(1)")
      });
    })

    context('when the admin try to add address(1)', () => {
      it('should revert', async() => {
        await expect(masterBarista.addPool("0x0000000000000000000000000000000000000001", 1000))
          .to.be.revertedWith("MasterBarista::addPool::_stakeToken must not be address(0) or address(1)")
      });
    })
    ...
    context('when the admin try to add duplicated token', () => {
      it('should revert', async() => {
        await expect(masterBarista.addPool(latteToken.address, 1000))
          .to.be.revertedWith("MasterBarista::addPool::_stakeToken duplicated")
      });
    })
    ...
    context('when admin try to set address(0)', () => {
      it('should revert', async() => {
        await expect(masterBarista.setPool(ethers.constants.AddressZero, 1000))
          .to.be.revertedWith("MasterBarista::setPool::_stakeToken must not be address(0) or address(1)")
      });
    })

    context('when admin try to add address(1)', () => {
      it('should revert', async() => {
        await expect(masterBarista.setPool("0x0000000000000000000000000000000000000001", 1000))
          .to.be.revertedWith("MasterBarista::setPool::_stakeToken must not be address(0) or address(1)")
      });
    })
  });
  ...
  describe('#setPoolAllocBps()', () => {
    context('when stake token is address(0)', () => {
      it('should revert', async () => {
        await expect(masterBarista.setPoolAllocBps(ethers.constants.AddressZero, 1000))
    ...
    context('when stake token is address(1)', () => {
      it('should revert', async () => {
        await expect(masterBarista.setPoolAllocBps("0x0000000000000000000000000000000000000001", 1000))
          .to.be.revertedWith("MasterBarista::setPoolAllocBps::_stakeToken must not be address(0) or address(1)")
      })
    })
    ...
    context("when the pool hasn't been set", () => {
      it('should revert', async() => {
        await expect(masterBarista.setPoolAllocBps(stakingTokens[0].address, 1000))
        .to.be.revertedWith("MasterBarista::setPoolAllocBps::pool hasn't been set")
      })
    })

    context('when alloc point equals to 1000', () => {
      it('should revert', async() => {
        masterBarista.addPool(stakingTokens[0].address, 1000)
        await expect(masterBarista.setPoolAllocBps(stakingTokens[0].address, 1000))
        .to.be.revertedWith("MasterBarista::setPoolallocBps::_allocBps must > 1000")
      })
    })

    context('when alloc point is less than 1000', () => {
      it('should revert', async () => {
        masterBarista.addPool(stakingTokens[0].address, 1000)
        await expect(masterBarista.setPoolAllocBps(stakingTokens[0].address, 999))
        .to.be.revertedWith("MasterBarista::setPoolallocBps::_allocBps must > 1000")
      })
    })
    ...
    context('when accumulated accum alloc bps is more than 10000', () => {
      it('should revert', async () => {
        ...
        await expect(masterBarista.setPoolAllocBps(stakingTokens[0].address, 6001))
        .to.be.revertedWith("MasterBarista::setPoolallocBps::accumAllocBps must < 10000")
      })
    })
    ...
    context('when accumulated accum alloc bps equals 10000', () => {
      it('should revert', async () => {
        masterBarista.addPool(stakingTokens[0].address, 1000)
        expect((await masterBarista.poolInfo(latteToken.address)).allocBps.add(BigNumber.from(6000))).to.eq(BigNumber.from(10000))
        // latte pool takes 40% since the initialization
        await expect(masterBarista.setPoolAllocBps(stakingTokens[0].address, 6000))
        .to.be.revertedWith("MasterBarista::setPoolallocBps::accumAllocBps must < 10000")
      })
    })
    ...
    context("when all parameters are valid", () => {
      it('should successfully update total alloc points', async () => {
        ...
        const lattePool = await masterBarista.poolInfo(latteToken.address)
        const stakeToken0Pool = await masterBarista.poolInfo(stakingTokens[0].address)
        const stakeToken1Pool = await masterBarista.poolInfo(stakingTokens[1].address)
        const totalAllocPoint = await masterBarista.totalAllocPoint()
  ...
  describe('#harvest()', () => {
    context('the caller is not a funder', () => {
      it('should revert', async() => {
        ...
        await expect(masterBarista["harvest(address,address)"](await deployer.getAddress(), latteToken.address))
    ...
    context('when harvesting through a stake token caller contract', () => {
      it('should notify a _onBeforeLock', async  () => {
        ...
        await expect(mockStakeTokenCaller.harvest(stakingTokens[0].address)).to.emit(mockStakeTokenCaller, 'OnBeforeLock')
      })
    })
  });
  ...
  describe('#addStakeTokenCallerContract', () => {
    context('when the pool does not allow adding a corresponding stake token caller', () => {
      it('should revert', async() => {
        ...
        await expect(masterBarista.addStakeTokenCallerContract(stakingTokens[0].address, stakeCallerContract)).to.be.revertedWith('')
      })
    })
  ...
  describe('#deposit()', () => {
    context('when the pool has been assigned as stakeTokenCallerPool through stakeTokenCallerAllowancePool', () => {
      context('when the caller is not a stake token caller contract', () => {
        it('should revert', async() => {
          ...
          await expect(masterBarista.deposit(await deployer.getAddress(), stakingTokens[0].address, ethers.utils.parseEther('100')))
      ...
      context('when the caller is a stake token caller contract',() => {
        it('should successfully deposit', async () => {
          ...
          await masterBarista.setStakeTokenCallerAllowancePool(stakingTokens[0].address, true)
          await masterBarista.addStakeTokenCallerContract(stakingTokens[0].address, stakeCallerContract)
          const tx = await masterBaristaAsAlice.deposit(await deployer.getAddress(), stakingTokens[0].address, ethers.utils.parseEther('100'))
      ...
      context('when the caller has been revoked after the first deposit', () => {
        it('should revert', async() => {
          ...
          await masterBarista.setStakeTokenCallerAllowancePool(stakingTokens[0].address, true)
          await masterBarista.addStakeTokenCallerContract(stakingTokens[0].address, stakeCallerContract)
          await masterBaristaAsAlice.deposit(await deployer.getAddress(), stakingTokens[0].address, ethers.utils.parseEther('100'))
          ...
          await expect(masterBaristaAsAlice.deposit(await deployer.getAddress(), stakingTokens[0].address, ethers.utils.parseEther('100')))
          .to.be.revertedWith("MasterBarista::onlyPermittedTokenFunder: caller is not permitted")
        })
      })
    })
    ...
    context("when the pool hasn't been assigned as stakeTokenCallerPool", () => {
      context('when the caller is not a _for', () => {
        it('should revert', async() => {
           await expect(masterBarista.deposit(await alice.getAddress(), stakingTokens[0].address, ethers.utils.parseEther('100')))
           .to.be.revertedWith("MasterBarista::onlyPermittedTokenFunder: caller is not permitted")
        })
      })
      ...
      context('when the caller is the same as _for', () => {
        it('should successfully deposit', async () => {
          ...
          await masterBarista.addPool(stakingTokens[0].address, 1000)
          const tx = await masterBarista.deposit(await deployer.getAddress(), stakingTokens[0].address, ethers.utils.parseEther('100'))
  ...
  describe('#depositLatte()', () => {
    context('when the pool has been assigned as stakeTokenCallerPool through stakeTokenCallerAllowancePool', () => {
      context('when the caller is not a stake token caller contract', () => {
        it('should revert', async() => {
          ...
          await expect(masterBarista.depositLatte(await deployer.getAddress(), ethers.utils.parseEther('100')))
          .to.be.revertedWith("MasterBarista::onlyPermittedTokenFunder: caller is not permitted")
        })
      })
      ...
      context('when the caller is a stake token caller contract', () => {
        it('should successfully deposit', async () => {
          ...
          await masterBarista.setStakeTokenCallerAllowancePool(latteToken.address, true)
          await masterBarista.addStakeTokenCallerContract(latteToken.address, stakeCallerContract)
          const tx = await masterBaristaAsAlice.depositLatte(await deployer.getAddress(), ethers.utils.parseEther('100'))
      ...
      context('when the caller has been revoked after the first deposit', () => {
        it('should revert', async() => {
          ...
          await expect(masterBaristaAsAlice.depositLatte(await deployer.getAddress(), ethers.utils.parseEther('100')))
          .to.be.revertedWith("MasterBarista::onlyPermittedTokenFunder: caller is not permitted")
        })
      })
    })
    ...
    context("when the pool hasn't been assigned as stakeTokenCallerPool", () => {
      context('when the caller is not a _for', () => {
        it('should revert', async() => {
           await expect(masterBarista.depositLatte(await alice.getAddress(), ethers.utils.parseEther('100')))
           .to.be.revertedWith("MasterBarista::onlyPermittedTokenFunder: caller is not permitted")
        })
      })
