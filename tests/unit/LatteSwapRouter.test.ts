...
describe("LatteSwapRouter", () => {
  ...
  describe("#removeLiquidityETH()", () => {
    it("should remove an added liquidity", async () => {
      ...
      const token0Amount = ethers.utils.parseEther("1");
      const bnbAmount = ethers.utils.parseEther("4");
      const [expectedSortedToken0Amount, expectedSortedToken1Amount] = sortsBefore(token0.address, wbnb.address)
        ? [token0Amount.sub(500), bnbAmount.sub(2000)]
        : [bnbAmount.sub(2000), token0Amount.sub(500)];
      const aliceAddress = await alice.getAddress();
      const deployerAddress = await deployer.getAddress();
      // sqrt(1*4) - MINIMUM_LIQUIDITY
      // 2*10^18 - 1000
  ...
  describe("#removeLiquidityWithPermit()", () => {
    it("should remove liquidity without any approval", async () => {
      ...
      await expect(
        latteSwapRouterAsAlice.removeLiquidityWithPermit(
          token0.address,
          token1.address,
          expectedLiquidity.sub(MINIMUM_LIQUIDITY),
          0,
  ...
  describe("#removeLiquidityETHWithPermit()", () => {
    it("should remove liquidity without any approval", async () => {
      ...
      await expect(
        latteSwapRouter.removeLiquidityETHWithPermit(
          token0.address,
          expectedLiquidity.sub(MINIMUM_LIQUIDITY),
          0,
          0,
  ...
  describe("#swapExactTokensForTokens()", () => {
    ...
    it("should emit expected event", async () => {
      await expect(
        latteSwapRouterAsAlice.swapExactTokensForTokens(
          swapAmount,
          0,
  ...
  describe("#swapTokensForExactTokens()", () => {
    ...
    it("should emit expected event", async () => {
      await expect(
        latteSwapRouterAsAlice.swapTokensForExactTokens(
          outputAmount,
          MaxUint256,
          [sortedToken0.address, sortedToken1.address],
          aliceAddress,
  ...
  describe("#swapExactETHForTokens()", () => {
    ...
    it("should emit expected event", async () => {
      await expect(
        latteSwapRouterAsAlice.swapExactETHForTokens(0, [wbnb.address, token0.address], aliceAddress, MaxUint256, {
          value: swapAmount,
        })
      )
        .to.emit(wbnb, "Transfer")
        ...
        .to.emit(nativePair, "Sync")
        .withArgs(
          nativePairToken0 === token0.address ? token0Amount.sub(expectedOutputAmount) : bnbAmount.add(swapAmount),
          nativePairToken0 === token0.address ? bnbAmount.add(swapAmount) : token0Amount.sub(expectedOutputAmount)
        )
    ...
    it("should swap a correct amount based on getAmountOut", async () => {
      ...
      await latteSwapRouterAsAlice.swapExactETHForTokens(0, [wbnb.address, token0.address], aliceAddress, MaxUint256, {
        value: swapAmount,
      });
  ...
  describe("#swapETHForExactTokens()", () => {
    ...
    it("should emit expected event", async () => {
      await expect(
        ...
        .to.emit(nativePair, "Sync")
        .withArgs(
          nativePairToken0 === token0.address ? token0Amount.sub(outputAmount) : bnbAmount.add(expectedSwapAmount),
          nativePairToken0 === token0.address ? bnbAmount.add(expectedSwapAmount) : token0Amount.sub(outputAmount)
        )
        .to.emit(nativePair, "Swap")
        .withArgs(
          latteSwapRouter.address,
    ...
    it("should swap a correct amount based on getAmountIn", async () => {
      ...
      expect(balanceBefore.sub(balanceAfter)).to.eq(
        expectedSwapAmount.add((await ethers.provider.getGasPrice()).mul(receipt.cumulativeGasUsed))
      );
    });
  });
  ...
  describe("#swapTokensForExactETH()", () => {
    ...
    it("should emit expected event", async () => {
      await expect(
        ...
        .to.emit(nativePair, "Sync")
        .withArgs(
          nativePairToken0 === token0.address ? token0Amount.add(expectedSwapAmount) : bnbAmount.sub(outputAmount),
          nativePairToken0 === token0.address ? bnbAmount.sub(outputAmount) : token0Amount.add(expectedSwapAmount)
        )
        .to.emit(nativePair, "Swap")
        .withArgs(
          latteSwapRouter.address,
  ...
  describe("#swapExactTokensForETH()", () => {
    ...
    it("should emit expected event", async () => {
      await expect(
        ...
        .to.emit(nativePair, "Sync")
        .withArgs(
          nativePairToken0 === token0.address ? token0Amount.add(swapAmount) : bnbAmount.sub(expectedOutputAmount),
          nativePairToken0 === token0.address ? bnbAmount.sub(expectedOutputAmount) : token0Amount.add(swapAmount)
        )
        .to.emit(nativePair, "Swap")
        .withArgs(
          latteSwapRouter.address,
    ...
    it("should swap a correct amount based on getAmountOut", async () => {
      ...
      expect(balanceAfter.sub(balanceBefore)).to.eq(
        expectedOutputAmount.sub((await ethers.provider.getGasPrice()).mul(receipt.cumulativeGasUsed))
      );
    });
  });
});
