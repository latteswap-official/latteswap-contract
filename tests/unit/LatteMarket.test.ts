...
import { LatteMarket, LatteMarket__factory, LatteNFT, MockLatteNFT, MockWBNB, SimpleToken, SimpleToken__factory, WNativeRelayer } from "../../typechain";
...
describe("LatteMarket", () => {
  ...
  describe('#readyToSellNFT()', () => {
    context("if the nft hasn't been supported", () => {
      it('should revert', async () => {
        await expect(latteMarket.readyToSellNFT(latteNFT.address, 0 , parseEther('10'), 1, startingBlock.add(100), startingBlock.add(101), stakingTokens[0].address)).to.revertedWith('LatteMarket::onlySupportedNFT::unsupported nft')
      })
    })
    ...
    context("if the seller is not a governance", () => {
      it('should revert', async () => {
        ...
        await expect(latteMarketAsAlice.readyToSellNFT(latteNFT.address, 0 , parseEther('10'), 1, startingBlock.add(100), startingBlock.add(101), stakingTokens[0].address)).to.revertedWith('LatteMarket::onlyGovernance::only GOVERNANCE role')
      })
    })
    ...
    context("if the nft to sell already in the auction", () => {
      it ('should revert', async () => {
        await latteMarket.setSupportNFT([latteNFT.address], true)
        await latteMarket.readyToStartAuction(latteNFT.address, 0 , parseEther('10'), 1, startingBlock.add(100), startingBlock.add(101), stakingTokens[0].address)
        await expect(latteMarketAsAlice.readyToSellNFT(latteNFT.address, 0 , parseEther('10'), 1, startingBlock.add(100), startingBlock.add(101), stakingTokens[0].address)).to.revertedWith('LatteMarket::onlyNonBiddingNFT::only selling token can be used here')
      })
    })
    ...
    context('when the quoteToken is address(0)', () => {
      it('should revert', async () => {
        await latteMarket.setSupportNFT([latteNFT.address], true)
        await expect(latteMarket.readyToSellNFT(latteNFT.address, 0 , parseEther('0'), 1, startingBlock.add(100), startingBlock.add(101), constants.AddressZero)).to.revertedWith('LatteMarket::_setCurrentPrice::invalid quote token')
      })
    })
    ...
    context('when invalid block', () => {
      context('when starting < current block', () => {
        it('should revert', async () => {
          await latteMarket.setSupportNFT([latteNFT.address], true)
          await expect(latteMarket.readyToSellNFT(latteNFT.address, 0 , parseEther('0'), 1, 0, 1, stakingTokens[0].address)).to.revertedWith('LatteMarket::_setLatteNFTMetadata::invalid start or end block')
        })
      })
      ...
      context('when starting >= end block', () => {
        it('should revert', async () => {
          await latteMarket.setSupportNFT([latteNFT.address], true)
          await expect(latteMarket.readyToSellNFT(latteNFT.address, 0 , parseEther('0'), 1, startingBlock.add(100), startingBlock.add(99), stakingTokens[0].address)).to.revertedWith('LatteMarket::_setLatteNFTMetadata::invalid start or end block')
        })
      })
    })
    ...
    it('should create a correct latteNFTMetadata', async () => {
      await latteMarket.setSupportNFT([latteNFT.address], true)
      await expect(latteMarket.readyToSellNFT(latteNFT.address, 0 , parseEther('10'), 1, startingBlock.add(100), startingBlock.add(101), stakingTokens[0].address))
      .to.emit(latteMarket, 'Ask').withArgs(await deployer.getAddress(), latteNFT.address, 0, parseEther('10'), stakingTokens[0].address)
      .to.emit(latteMarket, 'SetLatteNFTMetadata').withArgs(latteNFT.address, 0, 1, startingBlock.add(100), startingBlock.add(101))
  ...
  describe('#buyNFT', () => {
    context('when buying a non-support nft', () => {
      it('should revert', async () => {
        await expect(latteMarketAsAlice.buyNFT(latteNFT.address, 0, signatureAsAlice)).to.revertedWith('LatteMarket::onlySupportedNFT::unsupported nft')
      })
    })
    context('when the selling nft is not within the blockrange', () => {
      it('should revert', async () => {
        // selling phase
        await latteMarket.setSupportNFT([latteNFT.address], true)
        await latteMarket.readyToSellNFT(latteNFT.address, 0 , parseEther('10'), 1, startingBlock.add(100), startingBlock.add(101), stakingTokens[0].address)

        // buying phase
        await expect(latteMarketAsAlice.buyNFT(latteNFT.address, 0, signatureAsAlice)).to.revertedWith('LatteMarket::withinBlockRange:: invalid block number')
      })
    })
    ...
    context('when the selling nft is already in the auction', () => {
      it('should revert', async() => {
        await latteMarket.setSupportNFT([latteNFT.address], true)
        await latteMarket.readyToStartAuction(latteNFT.address, 0 , parseEther('10'), 1, startingBlock.add(3), startingBlock.add(10), stakingTokens[0].address)
        await expect(latteMarketAsAlice.buyNFT(latteNFT.address, 0, signatureAsAlice)).to.revertedWith('LatteMarket::onlyNonBiddingNFT::only selling token can be used here')
      })
    })
    ...
    context('when invalid signature', () => {
      it('should revert', async () => {
        await latteMarket.setSupportNFT([latteNFT.address], true)
        await latteMarket.readyToSellNFT(latteNFT.address, 0 , parseEther('10'), 1, startingBlock.add(3), startingBlock.add(10), stakingTokens[0].address)
        await expect(latteMarketAsAlice.buyNFT(latteNFT.address, 0, signatureAsDeployer)).to.revertedWith('LatteMarket::permit::INVALID_SIGNATURE')
      })
    })
    ...
    context('when reaching a maximum cap', () => {
      it('should revert', async () => {
        await latteMarket.setSupportNFT([latteNFT.address], true)
        await latteMarket.readyToSellNFT(latteNFT.address, 0 , parseEther('10'), 0, startingBlock.add(3), startingBlock.add(10), stakingTokens[0].address)
        await expect(latteMarketAsAlice.buyNFT(latteNFT.address, 0, signatureAsAlice)).to.revertedWith('LatteMarket::_decreaseCap::maximum mint cap reached')
      })
    })
      ...
      context('with msg.value', () => {
        context('when amount != msg.value', () => {
          it('should revert', async () => {
            ...
            // sell phase
            await latteMarket.setSupportNFT([latteNFT.address], true)
            await latteMarket.readyToSellNFT(latteNFT.address, 0 , parseEther('10'), 1, startingBlock.add(4), startingBlock.add(10), wbnb.address)
    
            // buy phase
            await expect(latteMarketAsAlice.buyNFT(latteNFT.address, 0, signatureAsAlice, {
              value: parseEther('11')
    ...
    context('when a token is not a native', () => {
      context('with msg.value', () => {
        it('should revert', async () => {
          ...
          // sell phase
          await latteMarket.setSupportNFT([latteNFT.address], true)
          await latteMarket.readyToSellNFT(latteNFT.address, 0 , parseEther('10'), 1, startingBlock.add(4), startingBlock.add(10), stakingTokens[0].address)
  ...
  describe('#buyBatchNFT', () => {
    context('when buying a non-support nft', () => {
      it('should revert', async () => {
        await expect(latteMarketAsAlice.buyBatchNFT(latteNFT.address, 0, 2, signatureAsAlice)).to.revertedWith('LatteMarket::onlySupportedNFT::unsupported nft')
      })
    })
    context('when the selling nft is not within the blockrange', () => {
      it('should revert', async () => {
        ...
        // buying phase
        await expect(latteMarketAsAlice.buyBatchNFT(latteNFT.address, 0, 2, signatureAsAlice)).to.revertedWith('LatteMarket::buyBatchNFT:: invalid block number')
      })
    })
    ...
    context('when the selling nft is already in the auction', () => {
      it('should revert', async() => {
        await latteMarket.setSupportNFT([latteNFT.address], true)
        await latteMarket.readyToStartAuction(latteNFT.address, 0 , parseEther('10'), 1, startingBlock.add(3), startingBlock.add(10), stakingTokens[0].address)
        await expect(latteMarketAsAlice.buyBatchNFT(latteNFT.address, 0, 2, signatureAsAlice)).to.revertedWith('LatteMarket::onlyNonBiddingNFT::only selling token can be used here')
      })
    })
    ...
    context('when invalid signature', () => {
      it('should revert', async () => {
        await latteMarket.setSupportNFT([latteNFT.address], true)
        await latteMarket.readyToSellNFT(latteNFT.address, 0 , parseEther('10'), 1, startingBlock.add(3), startingBlock.add(10), stakingTokens[0].address)
        await expect(latteMarketAsAlice.buyBatchNFT(latteNFT.address, 0, 2, signatureAsDeployer)).to.revertedWith('LatteMarket::permit::INVALID_SIGNATURE')
      })
    })
    ...
    context('when reaching a maximum cap', () => {
      it('should revert', async () => {
        await latteMarket.setSupportNFT([latteNFT.address], true)
        await latteMarket.readyToSellNFT(latteNFT.address, 0 , parseEther('10'), 0, startingBlock.add(3), startingBlock.add(10), stakingTokens[0].address)
        await expect(latteMarketAsAlice.buyBatchNFT(latteNFT.address, 0, 2, signatureAsAlice)).to.revertedWith('LatteMarket::_decreaseCap::maximum mint cap reached')
      })
    })
      ...
      context('with msg.value', () => {
        context('when amount != msg.value', () => {
          it('should revert', async () => {
            ...
            // buy phase
            await expect(latteMarketAsAlice.buyBatchNFT(latteNFT.address, 0, 2, signatureAsAlice, {
              value: parseEther('11')
            })).to.revertedWith('latteMarket::_safeWrap:: value != msg.value')
          })
        })
  ...
  describe('#setCurrentPrice', () => {
    context('when setting price of a token that is in an auction', () => {
      it('should revert', async () => {
        await latteMarket.setSupportNFT([latteNFT.address], true)
        await latteMarket.readyToStartAuction(latteNFT.address, 0 , parseEther('10'), 1, startingBlock.add(3), startingBlock.add(10), stakingTokens[0].address)
        await expect(latteMarket.setCurrentPrice(latteNFT.address, 0, parseEther('50'), wbnb.address)).to.revertedWith('LatteMarket::onlyNonBiddingNFT::only selling token can be used here')
      })
    })
    ...
    context('when setting price of a token if address(0)', () => {
      it('should revert', async () => {
        await latteMarket.setSupportNFT([latteNFT.address], true)
        await latteMarket.readyToStartAuction(latteNFT.address, 0 , parseEther('10'), 1, startingBlock.add(3), startingBlock.add(10), stakingTokens[0].address)
        await expect(latteMarket.setCurrentPrice(latteNFT.address, 0, parseEther('50'), constants.AddressZero)).to.revertedWith('LatteMarket::onlyNonBiddingNFT::only selling token can be used here')
      })
    })
  ...
  describe('#readyToStartAuction()', () => {
    context("if the nft hasn't been supported", () => {
      it('should revert', async () => {
        await expect(latteMarket.readyToStartAuction(latteNFT.address, 0 , parseEther('10'), 1, startingBlock.add(2), startingBlock.add(10), stakingTokens[0].address)).to.revertedWith('LatteMarket::onlySupportedNFT::unsupported nft')
      })
    })
    ...
    context("if the seller is not a governance", () => {
      it('should revert', async () => {
        await latteMarket.setSupportNFT([latteNFT.address], true)
        await expect(latteMarketAsAlice.readyToStartAuction(latteNFT.address, 0 , parseEther('10'), 1, startingBlock.add(3), startingBlock.add(10), stakingTokens[0].address)).to.revertedWith('LatteMarket::onlyGovernance::only GOVERNANCE role')
      })
    })

    it('should create a correct latteNFTMetadata', async () => {
      await latteMarket.setSupportNFT([latteNFT.address], true)
      await expect(latteMarket.readyToStartAuction(latteNFT.address, 0 , parseEther('10'), 1, startingBlock.add(3), startingBlock.add(10), stakingTokens[0].address))
      .to.emit(latteMarket, 'Ask').withArgs(await deployer.getAddress(), latteNFT.address, 0, parseEther('10'), stakingTokens[0].address)
      .to.emit(latteMarket, 'SetLatteNFTMetadata').withArgs(latteNFT.address, 0, 1, startingBlock.add(3), startingBlock.add(10))
      const metadata = await latteMarket.latteNFTMetadata(latteNFT.address, 0)
  ...
  describe('#bidNFT()', () =>  {
    context('when bidding a non-support nft', () => {
      it('should revert', async () => {
        await expect(latteMarketAsAlice.bidNFT(latteNFT.address, 0, parseEther('1'), signatureAsAlice)).to.revertedWith('LatteMarket::onlySupportedNFT::unsupported nft')
      })
    })
    context('when the bidding nft is not within the blockrange', () => {
      it('should revert', async () => {
        ...
        // buying phase
        await expect(latteMarketAsAlice.bidNFT(latteNFT.address, 0, parseEther('1'), signatureAsAlice)).to.revertedWith('LatteMarket::withinBlockRange:: invalid block number')
      })
    })
    ...
    context('when the bidding nft is not in the action', () => {
      it('should revert', async() => {
        await latteMarket.setSupportNFT([latteNFT.address], true)
        await latteMarket.readyToSellNFT(latteNFT.address, 0 , parseEther('10'), 1, startingBlock.add(3), startingBlock.add(10), stakingTokens[0].address)
        await expect(latteMarketAsAlice.bidNFT(latteNFT.address, 0, parseEther('1'), signatureAsAlice)).to.revertedWith('LatteMarket::onlyBiddingNFT::only bidding token can be used here')
      })
    })
    context('when the bidder is an owner', () => {
      it('should revert', async () => {
        await latteMarket.setSupportNFT([latteNFT.address], true)
        await latteMarket.readyToStartAuction(latteNFT.address, 0 , parseEther('10'), 1, startingBlock.add(3), startingBlock.add(10), stakingTokens[0].address)
        await expect(latteMarket.bidNFT(latteNFT.address, 0, parseEther('1'), signatureAsDeployer)).to.revertedWith('LatteMarket::_bidNFT::Owner cannot bid')
      })
    })
    context('when the input bid is < starting bid', () => {
      it('should revert', async () => {
        await latteMarket.setSupportNFT([latteNFT.address], true)
        await latteMarket.readyToStartAuction(latteNFT.address, 0 , parseEther('10'), 1, startingBlock.add(3), startingBlock.add(10), stakingTokens[0].address)
        await expect(latteMarketAsAlice.bidNFT(latteNFT.address, 0, parseEther('1'), signatureAsAlice)).to.revertedWith('LatteMarket::_bidNFT::price cannot be lower than or equal to the starting bid')
      })
    })
      ...
      context('with existing previous bid', () => {
        context('when the current input bid is < the prev bid', () => {
          it('should revert', async () => {
            ...
            // start auction phase
            await latteMarket.setSupportNFT([latteNFT.address], true)
            await latteMarket.readyToStartAuction(latteNFT.address, 0 , parseEther('10'), 1, startingBlock.add(4), startingBlock.add(10), wbnb.address)
            ...
            await expect(latteMarketAsBob.bidNFT(latteNFT.address, 0, parseEther('11'), signatureAsBob, {
              value: parseEther('11')
        ...
        context('when the prev bid and the request bid is the user', () => {
          it('should successfully add up a bid with requiring only an amount left', async () => {
            it('should successfully replace a bid with the previous bid returned to the user', async () => {
              ...
              expect(aliceBalAfter).to.eq(aliceBalBefore.sub(parseEther('11').add((await ethers.provider.getGasPrice()).mul(aliceGasUsed))))
              ...
              expect(bid.bidder).to.eq(await alice.getAddress())
              expect(bid.price).to.eq(parseEther('12'))
              expect(aliceBalAfter).to.eq(aliceBalBefore.add(parseEther('11')).sub(parseEther('12').add((await ethers.provider.getGasPrice()).mul(aliceGasUsed))))
            })
          })
        })
        context('when the prev bid and the request bid is not the same user', () => {
          it('should successfully replace a bid with the previous bid returned to the user', async () => {
            ...
            expect(bid.price).to.eq(parseEther('12'))
            expect(bobBalAfter).to.eq(bobBalBefore.sub(parseEther('12').add((await ethers.provider.getGasPrice()).mul(bobGasUsed))))
      ...
      context('with existing previous bid', () => {
        context('when the current input bid is < the prev bid', () => {
          it('should revert', async () => {
            ...
            // start auction phase
            await latteMarket.setSupportNFT([latteNFT.address], true)
            await latteMarket.readyToStartAuction(latteNFT.address, 0 , parseEther('10'), 1, startingBlock.add(4), startingBlock.add(10), stakingTokens[0].address)

             //mint staking tokens
            await stakingTokens[0].mint(await alice.getAddress(), parseEther('100'))
            await SimpleToken__factory.connect(stakingTokens[0].address, alice).approve(latteMarket.address,  parseEther('12'))
        ...
        context('when the prev bid and the request bid is the user', () => {
          it('should successfully add up a bid with requiring only an amount left', async () => {
            it('should successfully replace a bid with the previous bid returned to the user', async () => {
              ...
              //mint staking tokens
              await stakingTokens[0].mint(await alice.getAddress(), parseEther('100'))
              await SimpleToken__factory.connect(stakingTokens[0].address, alice).approve(latteMarket.address,  parseEther('11'))
              ...
              await SimpleToken__factory.connect(stakingTokens[0].address, alice).approve(latteMarket.address,  parseEther('1'))
        ...
        context('when the prev bid and the request bid is not the same user', () => {
          it('should successfully replace a bid with the previous bid returned to the user', async () => {
            ...
            await SimpleToken__factory.connect(stakingTokens[0].address, alice).approve(latteMarket.address,  parseEther('11'))
            await SimpleToken__factory.connect(stakingTokens[0].address, bob).approve(latteMarket.address,  parseEther('12'))
  ...
  describe('#concludeAuction()', () => {
    context("if the nft hasn't been supported", () => {
      it('should revert', async () => {
        await expect(latteMarket.concludeAuction(latteNFT.address, 0 )).to.revertedWith('LatteMarket::onlySupportedNFT::unsupported nft')
      })
    })
    ...
    context("when the seller is not a governance", () => {
      it('should revert', async () => {
        await latteMarket.setSupportNFT([latteNFT.address], true)
        await latteMarket.readyToStartAuction(latteNFT.address, 0 , parseEther('10'), 1, startingBlock.add(3), startingBlock.add(10), stakingTokens[0].address)
        await expect(latteMarketAsAlice.concludeAuction(latteNFT.address, 0)).to.revertedWith('LatteMarket::onlyGovernance::only GOVERNANCE role')
      })
    })
    ...
    context("when the nft to sell not in the auction", () => {
      it ('should revert', async () => {
        await latteMarket.setSupportNFT([latteNFT.address], true)
        await latteMarket.readyToSellNFT(latteNFT.address, 0 , parseEther('10'), 1, startingBlock.add(3), startingBlock.add(10), stakingTokens[0].address)
        await expect(latteMarketAsAlice.concludeAuction(latteNFT.address, 0)).to.revertedWith('LatteMarket::onlyBiddingNFT::only bidding token can be used here')
      })
    })
    ...
    context('when the block number isn NOT ready to be concluded', () => {
      it('should revert', async () => {
        ...
        await latteMarket.setSupportNFT([latteNFT.address], true)
        await latteMarket.readyToStartAuction(latteNFT.address, 0 , parseEther('10'), 1, startingBlock.add(3), startingBlock.add(10), stakingTokens[0].address)
        await expect(latteMarket.concludeAuction(latteNFT.address, 0)).to.revertedWith('LatteMarket::_concludeAuction::Unable to conclude auction now, bad block number')
      })
    })
    ...
    context('when the bidder is not existed', () => {
      it('should revert', async () => {
        await latteMarket.setSupportNFT([latteNFT.address], true)
        await latteMarket.readyToStartAuction(latteNFT.address, 0 , parseEther('10'), 1, startingBlock.add(3), startingBlock.add(4), stakingTokens[0].address)
        await advanceBlockTo(startingBlock.add(4).toNumber());
        await expect(latteMarket.concludeAuction(latteNFT.address, 0)).to.revertedWith('LatteMarket::_concludeAuction::Bidder does not exist')
      })
    })
    ...
    context('when reaching a maximum cap', () => {
      it('should revert', async () => {
        ...
        // start auction phase
        await latteMarket.setSupportNFT([latteNFT.address], true)
        await latteMarket.readyToStartAuction(latteNFT.address, 0 , parseEther('10'), 0, startingBlock.add(4), startingBlock.add(5), wbnb.address)
        ...
        // conclude phase
        await expect(latteMarket.concludeAuction(latteNFT.address, 0)).to.revertedWith('LatteMarket::_decreaseCap::maximum mint cap reached')
      })
    })
    ...
    context('when cancel auction', () => {
      it('should return all bids back to the bidder with reverted message if continue to either bid or cancel auction', async () => {
        ...
        // start auction phase
        await latteMarket.setSupportNFT([latteNFT.address], true)
        await latteMarket.readyToStartAuction(latteNFT.address, 0 , parseEther('10'), 1, startingBlock.add(4), startingBlock.add(6), stakingTokens[0].address)
        await stakingTokens[0].mint(await alice.getAddress(), parseEther('100'))
        await SimpleToken__factory.connect(stakingTokens[0].address, alice).approve(latteMarket.address, parseEther('11'))
        // bid phase
        await latteMarketAsAlice.bidNFT(latteNFT.address, 0, parseEther('11'), signatureAsAlice)
        ...
        // conclude phase
        await expect(latteMarket.concludeAuction(latteNFT.address, 0)).to.revertedWith("LatteMarket::onlyBiddingNFT::only bidding token can be used here")
        await SimpleToken__factory.connect(stakingTokens[0].address, alice).approve(latteMarket.address, parseEther('11'))
        // bid phase
        await expect(latteMarketAsAlice.bidNFT(latteNFT.address, 0, parseEther('11'), signatureAsAlice)).to.revertedWith('LatteMarket::withinBlockRange:: invalid block number')
      })
    })
    ...
    it ('should successfully conclude an auction', async () => {
      ...
      // start auction phase
      await latteMarket.setSupportNFT([latteNFT.address], true)
      await latteMarket.readyToStartAuction(latteNFT.address, 0 , parseEther('10'), 1, startingBlock.add(4), startingBlock.add(5), wbnb.address)
  ...
  describe('#setLatteNFTMetadata', () => {
    context('when setting unsupported nft', () => {
      it ('should revert', async () => {
        ...
        await expect(latteMarket.setLatteNFTMetadata([{
