...
async function main() {
  ...
  const router: LatteSwapRouter = LatteSwapRouter__factory.connect(
    ROUTER_ADDRESS, deployer);
  ...
  for (const pair of PAIRS) {
    ...
    if (pair.TOKEN0 === wbnbAddress) {
      const tx = await router.addLiquidityETH(
        token1.address,
        pair.TOKEN1_PRICE,
        '0', '0', deployer.address, FOREVER,
        { value: pair.TOKEN0_PRICE, gasLimit: 5000000 }
      )
      ...
      const tx = await router.addLiquidityETH(
        ...
        pair.TOKEN0_PRICE,
        '0', '0', deployer.address, FOREVER,
        { value: pair.TOKEN1_PRICE, gasLimit: 5000000 }
      )
      console.log(`>> tx hash addLiquidityETH ${tx.hash}`)
    } else {
