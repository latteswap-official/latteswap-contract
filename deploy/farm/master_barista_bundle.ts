...
import { BeanBag, BeanBag__factory, LATTE, LATTE__factory, MasterBarista, MasterBarista__factory } from '../../typechain';
  ...
  */
  ...
  withNetworkFile(async() => {
    ...
    const latte = LATTE__factory.connect(
      (await deployments.get('LATTE')).address, (await ethers.getSigners())[0]
    ) as LATTE
    ...
    const bean = BeanBag__factory.connect(
      (await deployments.get('BeanBag')).address, (await ethers.getSigners())[0]
    ) as BeanBag
    ...
    console.log(`>> Set MasterBarista bonus to BONUS_MULTIPLIER: "${BONUS_MULTIPLIER}", BONUS_END_BLOCK: "${BONUS_END_BLOCK}", LOCK_BPS: ${BONUS_LOCK_BPS}`)
